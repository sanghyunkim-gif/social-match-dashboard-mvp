...
- **미완료 데이터 제외**: 오늘 날짜(${new Date().toISOString().slice(0, 10)}) 기준으로 아직 종료되지 않은 주차(미래 날짜가 포함된 주차)의 데이터는 수집이 완료되지 않았으므로 분석에서 제외하세요. 해당 주차 데이터가 있더라도 "해당 주차는 아직 데이터 수집 중이므로 분석에서 제외했습니다"라고 명시하세요.
